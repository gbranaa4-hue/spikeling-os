...
- [x] **Milestone 30**: real per-process address space isolation,
      ...
      and independently re-verified against the real project tree.
- [ ] Everything genuinely open-ended from here (a general program
      loader, a wider syscall surface, per-process heaps/file
      descriptors -- the next steps in the prerequisite chain Milestone
      27/30 started, for running software not built specifically for
      spikeling-os; recursive subdirectory nesting; rmdir) should be
      prioritized by what the research actually needs, not by trying to
      match any existing OS's
      feature list.
